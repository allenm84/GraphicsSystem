...
{
  public static class Ratcliff
  {
    const float EPSILON = 0.0000000001f;

    public static int[] Triangulate(IEnumerable<GsVector> inputVertices)
    {
      List<GsVector> contour = new List<GsVector>(inputVertices.Distinct());
    ...
    private static float Area(List<GsVector> contour)
    {
      int n = contour.Count;
      float A = 0.0f;
    ...
    private static bool Snip(List<GsVector> contour, int u, int v, int w, int n, int[] V)
    {
      int p;
